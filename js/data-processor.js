...
class DataProcessorFinal {
  ...
  processData(filters) {
    ...
    const kpiMetrics = this.calculateKPIMetrics(
      processedRents,
      salary,
      filters,
    );
    ...
    return {
      ...
      summary: {
        ...
        barriosUnicos: [...new Set(processedRents.map((r) => r.neighborhood))]
          .length,
        esfuerzoPromedio:
          processedRents.reduce((sum, r) => sum + r.effort, 0) /
          processedRents.length,
      },
    };
  }
  ...
  getProfileSalary(category, level) {
    ...
    if (category === "all" && level === "all") {
      const total = this.profilesData.reduce(
        (sum, profile) => sum + profile.Salary,
        0,
      );
    ...
    if (category === "all") {
      ...
      const total = levelProfiles.reduce(
        (sum, profile) => sum + profile.Salary,
        0,
      );
    ...
    if (level === "all") {
      const categoryProfiles = this.profilesData.filter(
        (p) => p.category === category,
      );
      if (categoryProfiles.length === 0) return 0;
      const total = categoryProfiles.reduce(
        (sum, profile) => sum + profile.Salary,
        0,
      );
  ...
  calculateKPIMetrics(processedRents, salary, filters) {
    ...
    const averageRent =
      processedRents.reduce((sum, rent) => sum + rent.price, 0) /
      processedRents.length;
